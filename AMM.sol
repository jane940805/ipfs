...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
        ...
        // Apply the fee and calculate the effective amount to trade
        uint256 effectiveSellAmount = (sellAmount * (10000 - feebps)) / 10000;
        
        // Calculate amount of tokens to be received using the constant product formula
        uint256 buyAmount = (buyReserve * effectiveSellAmount) / (sellReserve + effectiveSellAmount);

        // Transfer `sellAmount` of `sellToken` from the user
        ERC20(sellToken).transferFrom(msg.sender, address(this), sellAmount);
        
        // Transfer calculated `buyAmount` of `buyToken` to the user
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
        ...
        // Transfer tokens from the provider to the AMM contract
