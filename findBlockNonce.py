...
def mine_block(k, prev_hash, rand_lines):
    ...
    # Convert transaction lines to a single bytes object
    ...
    while True:
        ...
        # Concatenate prev_hash, transactions, and nonce_bytes
        combined = prev_hash + transactions + nonce_bytes
    ...
    # Convert the nonce to bytes and return it
    nonce_bytes = str(nonce).encode('utf-8')
...
if __name__ == '__main__':
    ...
    diff = 20
    prev_hash = hashlib.sha256(b'previous_block').digest()  # Simulating a previous block hash
